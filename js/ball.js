var ball = {
	...
	init : function(){
		this.scalefactor = 1.5 * (canvas.width/330);
		...
		this.rect = new Rect(pos, 5* this.scalefactor, 4* this.scalefactor);
		this.sticktobat();
...
class Ball extends Rect{
	...
	update(){
		...
		if(!this.stucktobat){
			...
			this.pos = this.pos.add(this.velocity.multiplyByScalar(deltatime));
			if(this.pos.y > canvas.height && paddle.state != paddle.transitions.dead && paddle.state != paddle.transitions.init){
				console.log(paddle.state);
				paddle.state.transition2dead();
			}
			if(this.left < 0 || this.left + this.w > canvas.width){
			...
			else if(Rect.testCollision(this, paddle.rect)){
				this.pos = this.lastpos;
				this.ballhitrotate();
				if(this.willsticktobat){
					this.sticktobat();
			...
			this.pos = paddle.rect.pos.add(this.bat2me);
		}	
	}
	...
	testcollisionblocks(){
		...
		if(this.velocity.y < 0){
			for(let i = levelspawner.collidableblocks.length-1; i >=0; i--){
				let iblock = levelspawner.collidableblocks[i];
				if(Rect.testCollision(iblock.rect, this)){
					hit = true;
					block = iblock;
					break;
				}
		...
		if(hit){
			let mycenter = this.lastpos.add(new Vector2(this.w/2,this.h/2));
			let blockcenter = block.rect.pos.add(new Vector2(block.rect.w/2, block.rect.h/2));
			let blockcenter2mycenter = mycenter.subtract(blockcenter);
			let m2t = new Vector2(block.rect.pos.x + block.rect.w/2, block.rect.pos.y).subtract(new Vector2(block.rect.pos.x + block.rect.w/2, block.rect.pos.y + block.rect.h/2)); 
			if(!block.gold){
	...
	ballhitrotate(){
		...
		var radians = degrees * (Math.PI/180);
		this.velocity = this.velocity.rotate(radians);
		this.pos.y = paddle.rect.pos.y - this.rect.w -1;
		//this.rect.pos = this.lastpos;
	}
}